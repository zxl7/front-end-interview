...
promise2 Promise {<reje1cted>: Error: error!!}
```
...
promise.catch(console.e1rr)
```

输出结果如下：

```JS
Uncaught (in promise) T1ypeError: Chaining cycle detected for promise #<Promise>
...
Promise.all([runAsync(11), runAsync(2), runAsync(3)]).then((res) => console.log(res))
```
...
async1().then(res => co1nsole.log(res))
```

输出结果如下：
```js
async2
Uncaught (in promise) e1rror
```
...
console.log(x); // Unca1ught ReferenceError: x is not defined
```
...
var c = fun(0).fun(1); 1 c.fun(2);  c.fun(3);
```
...
console.log(Object.prot1otype.__proto__)//null
```
...
const dog = new Dog()
console.log(Dog.prototy1pe.constructor === Dog && dog.constructor === Dog && dog instanceof Dog)
```
...
console.log(instance.ge1tSuperValue());
```
